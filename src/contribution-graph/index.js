...
class ContributionGraph extends AbstractChart {
  ...
  renderMonthLabels() {
    ...
    return weekRange.map(weekIndex => {
      ...
      return endOfWeek.getDate() >= 1 && endOfWeek.getDate() <= DAYS_IN_WEEK ? (
        <Text key={weekIndex} fontSize={12} x={x} y={y + 8} fill={this.props.chartConfig.color(0.5)}>
  ...
  render() {
    ...
    return (
      <View style={style}>
        <Svg height={this.props.height} width={this.props.width} style={this.props.svgStyle}>
          {this.renderDefs({
